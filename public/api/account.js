...
$('form').submit(function(e) {
    ...
    $.ajax({
        url: `${api_url}/employee/${user_id}/update`,
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        success: function(result) {
            addLoading()
            customAlert('success', 'Personal info updated')
            localStorage.setItem('photo', result.data.profile_photo_url)
            $('.avatar').attr('src', result.data.profile_photo_url)
        },
        error: function(xhr) {
